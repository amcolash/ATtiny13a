...
void LCD_init(void){			//initialize in 8 bit mode
  ...
  Send_zero();			//Send 0 0 0 0 - 1 1 0 0 command: Display ON, Cursor OFF, No blinking
  Send_zero();
  Send_zero();
  Send_zero();
  Send_one();
...
void Print_L(void){		//send 0100-1100 Data mode (RS=HIGH)
  Send_zero();
  Send_one();
  Send_zero();
  Send_zero();
...
void Print_O(void){		//send 0100-1111 Data mode (RS=High)
  Send_zero();
  Send_one();
  Send_zero();
  Send_zero();
